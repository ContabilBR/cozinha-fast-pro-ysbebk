...
export default function PedidoDetailScreen() {
  ...
  const fetchPedido = useCallback(async () => {
    console.log("[PedidoDetail] Fetching pedido:", id);
    try {
  ...
  const handleRefresh = () => {
    console.log("[PedidoDetail] Manual refresh");
    setRefreshing(true);
    fetchPedido();
  };

  const handleStatusUpdate = async (status: PedidoStatus) => {
    console.log("[PedidoDetail] Update status:", id, "->", status);
    setUpdating(true);
    try {
      await apiPatch(`/api/pedidos/${id}/status`, { status });
      console.log("[PedidoDetail] Status updated");
      await fetchPedido();
    } catch (e: any) {
      console.error("[PedidoDetail] Status update error:", e);
    } finally {
      setUpdating(false);
    }
  };
        ...
        {loading ? (
          ...
          >
                    ...
                    <View style={{ alignItems: "center", gap: 4 }}>
                      <View
                        style={{
                          ...
                          height: 20,
                          borderRadius: 10,
                          backgroundColor: isDone ? color : COLORS.surfaceSecondary,
                          alignItems: "center",
            ...
            {pedido?.status === "pronto" && role === "garcom" && (
              <AnimatedPressable
                onPress={() => handleStatusUpdate("entregue")}
                disabled={updating}
                style={{
                ...
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                  {ADMIN_STATUSES.filter((s) => s.status !== pedido?.status).map((s) => {
                    ...
                    return (
                      <AnimatedPressable
                        key={s.status}
                        onPress={() => handleStatusUpdate(s.status)}
                        disabled={updating}
                        style={{
                      ...
                      >
                        <Text style={{ fontFamily: "Outfit_600SemiBold", fontSize: 13, color: sc }}>
                          {s.label}
                        </Text>
                      </AnimatedPressable>
                    );
