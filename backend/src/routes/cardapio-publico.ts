...
// GET /api/public/restaurantes — lista restaurantes ativos (sem auth)
  ...
  app.fastify.get("/cardapio", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const html = '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + restNome + '</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,system-ui,sans-serif;background:#FAF7F4;color:#1a1a1a;max-width:480px;margin:0 auto}.hd{background:#E8521A;color:#fff;padding:20px 16px;text-align:center}.hd h1{font-size:22px}.hd p{font-size:14px;opacity:.85;margin-top:4px}.ct{padding:12px 16px;background:rgba(232,82,26,.1);font-size:13px;font-weight:600;color:#E8521A;text-transform:uppercase;position:sticky;top:0}.it{display:flex;justify-content:space-between;align-items:center;padding:14px 16px;border-bottom:.5px solid #eee;cursor:pointer}.it:active{background:#f0f0f0}.pr{font-size:16px;font-weight:700;color:#E8521A}.cr{position:fixed;bottom:0;left:0;right:0;max-width:480px;margin:0 auto;background:#E8521A;color:#fff;padding:16px;display:none;cursor:pointer}.md{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:10}.mc{position:absolute;bottom:0;left:0;right:0;max-width:480px;margin:0 auto;background:#fff;border-radius:16px 16px 0 0;padding:20px;max-height:80vh;overflow-y:auto}.bt{width:100%;padding:16px;border:none;border-radius:12px;font-size:16px;font-weight:600;cursor:pointer;margin-top:12px}.bp{background:#E8521A;color:#fff}.bo{background:#fff;border:1.5px solid #E8521A;color:#E8521A}.qi{display:flex;align-items:center;gap:12px}.qi button{width:32px;height:32px;border-radius:50%;border:1.5px solid #E8521A;background:#fff;color:#E8521A;font-size:18px;cursor:pointer}</style></head><body><div class="hd"><h1>' + restNome + '</h1><p>' + (m !== "0" ? "Mesa " + m : "Cardapio Digital") + '</p></div><div id="cd"><div style="text-align:center;padding:60px;color:#888">Carregando...</div></div><div class="cr" id="cr" onclick="oc()"><div style="display:flex;justify-content:space-between;align-items:center"><div style="display:flex;align-items:center;gap:10px"><div style="background:#fff;color:#E8521A;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-weight:700" id="cc">0</div><span>Ver pedido</span></div><div style="font-size:18px;font-weight:700" id="ctt">R$ 0</div></div></div><div class="md" id="md" onclick="event.target===this&&cl()"><div class="mc" id="mc"></div></div><script>var R="' + r + '",M=' + m + ',D=[],C=[];function fmt(v){return"R$ "+v.toFixed(2).replace(".",",")}function init(){if(!R){document.getElementById("cd").innerHTML="QR Code invalido";return}fetch("/api/public/cardapio/"+R).then(function(r){return r.json()}).then(function(d){D=d.cardapio||[];ren()}).catch(function(){document.getElementById("cd").innerHTML="Erro ao carregar"})}function ren(){var h="";D.forEach(function(c){h+="<div class=ct>"+c.categoria.nome+"</div>";c.pratos.forEach(function(p){h+="<div class=it onclick=\\"ad(\'"+p.id+"\',\'"+p.nome.replace(/\'/g,"")+"\',"+p.preco+")\\"><div><div style=font-size:15px;font-weight:500>"+p.nome+"</div>"+(p.descricao?"<div style=font-size:12px;color:#888;margin-top:2px>"+p.descricao+"</div>":"")+"</div><div class=pr>"+fmt(p.preco)+"</div></div>"})});document.getElementById("cd").innerHTML=h||"Nenhum prato"}function ad(id,n,p){var e=C.find(function(i){return i.id===id});if(e)e.q++;else C.push({id:id,n:n,p:p,q:1});uc()}function uc(){var t=0,c=0;C.forEach(function(i){t+=i.p*i.q;c+=i.q});document.getElementById("cr").style.display=c>0?"block":"none";document.getElementById("cc").textContent=c;document.getElementById("ctt").textContent=fmt(t)}function oc(){var t=0;C.forEach(function(i){t+=i.p*i.q});var h="<h2 style=font-size:18px;margin-bottom:16px>Seu Pedido</h2>";C.forEach(function(i,x){h+="<div style=display:flex;justify-content:space-between;align-items:center;padding:10px_0;border-bottom:.5px_solid_#eee><div><div style=font-weight:500>"+i.n+"</div><div style=color:#888;font-size:13px>"+fmt(i.p)+"</div></div><div class=qi><button onclick=cq("+x+",-1)>-</button><span style=font-size:16px;font-weight:600;min-width:20px;text-align:center>"+i.q+"</span><button onclick=cq("+x+",1)>+</button></div></div>"});h+="<div style=display:flex;justify-content:space-between;padding:14px_0;font-size:18px;font-weight:700><span>Total</span><span style=color:#E8521A>"+fmt(t)+"</span></div>";h+="<input style=width:100%;padding:12px;border:1.5px_solid_#ddd;border-radius:10px;font-size:15px;margin-bottom:8px id=cn placeholder=\\"Seu nome (opcional)\\">";h+="<button class=\\"bt bp\\" onclick=ep()>Enviar pedido</button><button class=\\"bt bo\\" onclick=cl() style=margin-top:8px>Continuar pedindo</button>";document.getElementById("mc").innerHTML=h;document.getElementById("md").style.display="block"}function cl(){document.getElementById("md").style.display="none"}function cq(x,d){C[x].q+=d;if(C[x].q<=0)C.splice(x,1);uc();if(C.length>0)oc();else cl()}function ep(){var n=document.getElementById("cn");var nome=n?n.value:"";fetch("/api/public/pedido",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({restaurante_id:R,mesa_numero:M,cliente_nome:nome||undefined,itens:C.map(function(i){return{prato_id:i.id,quantidade:i.q}})})}).then(function(r){return r.json().then(function(d){if(r.ok){document.getElementById("md").style.display="none";document.getElementById("cr").style.display="none";document.getElementById("cd").innerHTML="<div style=text-align:center;padding:40px><div style=font-size:48px>OK</div><h2 style=color:#22C55E;font-size:22px;margin-top:16px>Pedido enviado!</h2><p style=color:#888;margin-top:8px>"+d.mensagem+"</p><button class=\\"bt bp\\" onclick=\\"C=[];ren();uc()\\" style=margin-top:20px>Novo pedido</button></div>"}else{alert(d.error||"Erro")}})}).catch(function(){alert("Erro de conexao")})}init()</script></body></html>';
