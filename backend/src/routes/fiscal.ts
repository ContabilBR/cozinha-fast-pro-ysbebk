...
export function registerFiscalRoutes(app: App) {
  ...
  // GET /api/fiscal/diagnostico — Diagnostic information about Focus NFE integration
  app.fastify.get(
    "/api/fiscal/diagnostico",
    {
      schema: {
        description: "Get diagnostic information about Focus NFE integration configuration",
        tags: ["fiscal"],
        response: {
          200: {
            ...
            type: "object",
            properties: {
              tokenLength: { type: "integer", description: "Length of the Focus NFE API token" },
              tokenFirst3: { type: "string", description: "First 3 characters of the token" },
              tokenLast3: { type: "string", description: "Last 3 characters of the token" },
              focusEnv: { type: "string", description: "Focus NFE environment setting" },
              baseUrl: { type: "string", description: "Base URL being used for Focus NFE API calls" },
              nodeEnv: { type: "string", description: "Current Node environment" },
            },
          },
          500: {
            description: "Internal server error",
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const focusEnv = process.env.FOCUS_NFE_ENV || "production";
        const baseUrl = FOCUS_BASE_URL;
        const nodeEnv = process.env.NODE_ENV || "development";

        const diagnostico = {
          tokenLength: token.length,
          tokenFirst3: token.substring(0, 3),
          tokenLast3: token.substring(token.length - 3),
          focusEnv,
          baseUrl,
          nodeEnv,
        };

        app.logger.info(diagnostico, "Focus NFE diagnostics retrieved");
        return reply.code(200).send(diagnostico);
      } catch (err: any) {
        app.logger.error({ err }, "Failed to retrieve diagnostics");
        return reply.code(500).send({ error: err.message });
      }
    }
  );
