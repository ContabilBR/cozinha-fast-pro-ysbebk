...
export function registerAdminRoutes(app: App) {
  ...
  // GET /admin/check-focus-token — diagnostic endpoint (no auth required)
  app.fastify.get(
    "/admin/check-focus-token",
    {
      schema: {
        description: "Diagnostic check for FOCUS_NFE_TOKEN environment variable (no authentication required)",
        tags: ["admin"],
        response: {
          200: {
            type: "object",
            properties: {
              ...
              trimmedLength: { type: "number", nullable: true },
              hasLeadingWhitespace: { type: "boolean", nullable: true },
              hasTrailingWhitespace: { type: "boolean", nullable: true },
              hasNewline: { type: "boolean", nullable: true },
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        app.logger.info({}, "Check FOCUS token diagnostic");

        const varNames = ["FOCUS_NFE_TOKEN", "SPECULAR_SECRET_FOCUS_NFE_TOKEN", "SECRET_FOCUS_NFE_TOKEN"];
        ...
        if (!value) {
          app.logger.info({}, "No FOCUS token found in any env variable");
          return reply.code(200).send({ exists: false, source: null });
        }
        ...
        app.logger.info({ source, length: value.length }, "FOCUS token found");

        return reply.code(200).send(result);
      } catch (err) {
        app.logger.error({ err }, "FOCUS token check failed");
        return reply.code(500).send({ error: "Check failed: " + (err as any).message });
      }
    }
  );
}
